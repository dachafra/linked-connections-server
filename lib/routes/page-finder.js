...
const server_extension = utils.serverConfig.extension;
...
router.get('/'+server_extension+'/:agency/connections', async (req, res) => {
    let t0 = new Date();
    ...
    if (req.url.indexOf('connections/') >= 0) {
        res.location(server_extension+'/' + agency + '/connections?departureTime=' + departureTime.toISOString());
        res.status(302).send();
        return;
    }
    ...
    try {
        ...
        if (req.headers['accept-datetime'] !== undefined) {
            ...
            res.location('/memento/' + server_extension + '/' + agency + '?version=' + closest_version[0] + '&departureTime=' + new Date(closest_version[1]).toISOString());
            res.set({
                'Vary': 'Accept-Encoding, Accept-Datetime',
                'Link': '<' + host + server_extension + '/' + agency + '/connections?departureTime=' + departureTime.toISOString() + '>; rel=\"original timegate\"'
            });
        ...
        if (departureTime.getTime() !== found_fragment) {
            res.location('/' + server_extension + '/' + agency + '/connections?departureTime=' + ff.toISOString());
            res.status(302).send();
            return;
        }
