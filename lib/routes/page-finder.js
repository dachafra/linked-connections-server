...
router.get('/'+extension+':agency/connections', async (req, res) => {
    ...
    try {
        ...
        if (req.headers['accept-datetime'] !== undefined) {
            ...
            res.location('/memento/'+ extension + agency + '?version=' + closest_version[0] + '&departureTime=' + new Date(closest_version[1]).toISOString());
            res.set({
        ...
        if (departureTime.getTime() !== found_fragment) {
            res.location('/' + extension + agency + '/connections?departureTime=' + ff.toISOString());
            res.status(302).send();
            return;
        }
